...
class PivotPoints(IndicatorType):
  ...
  def __init__(self, symbol):
    ...
    self.CSVBuilder = CSVBuilder(['Date', 'Action', 'Price', 'S2', 'S1', 'P', 'R1', 'R2'], 'PivotPoints ' + symbol)
  ...
  def run(self, candle: Dict[Any, Any]):
    ...
    self.CSVBuilder.write([datetime.fromtimestamp(candle['datetime'] / 1000).isoformat(' '), Action, candle['close'], "{0:.2f}".format(S2), "{0:.2f}".format(S1), "{0:.2f}".format(P), "{0:.2f}".format(R1), "{0:.2f}".format(R2)])
  ...
  def getHLC(self, currentTime: datetime) -> Any:
    ...
    while (datetime.now().timestamp() - (res['candles'][current]["datetime"] / 1000) < 86400):
      current -= 1
